...
const filterd_data = data.filter(d=>d.year_save==selected_year);
  // Create a set of unique months from the focused data and sort it
let month = new Set(filterd_data.map(function(d) {
  return d.month_save;
}));
month = [...month].sort(
  (a, b) => {
...
const filterd_data = data.filter(
